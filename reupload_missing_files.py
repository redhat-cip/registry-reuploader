...
parser = argparse.ArgumentParser(
    description="Search for missing container " "images blobs in swift and upload them"
)
...
def get_blob_content_v1(image, blob):
    ...
    url = "https://docker-registry.engineering.redhat.com/v2/%s/blobs/sha256:%s" % (
        image,
        blob,
    )  # noqa
...
def get_blob_content_v2(image, blob):
    ...
    token = requests.get(
        "%s/v2/auth?scope=repository:%s/%s:pull" % (url, ns, image), verify=False
    ).json()["token"]
    headers = {"Authorization": "Bearer %s" % token}

    r = requests.get(
        "%s/v2/%s/%s/blobs/sha256:%s" % (url, ns, image, blob),
        headers=headers,
        verify=False,
    )
    return r
...
def reupload_blobs():
    for path, blob in empty_blobs():
        ...
        conn.object_store.upload_object(
            container="dci_registry", name=path, data=r.content
        )
